...
l_module.start = function (config, onDone) {
	...
	*/
	...
	if (config.secured === true && SR.Keys) {
		...
		let express_port = UTIL.getProjectPort('PORT_INC_EXPRESS_S');
		let options = {
		...
		let server = SR.https.createServer(options, app).listen(express_port, function() {
			LOG.warn('Express server listening securely on port ' + express_port, l_name);
		});
				
	} else {
		let express_port = UTIL.getProjectPort('PORT_INC_EXPRESS');
		let server = SR.http.createServer(app).listen(express_port, function() {
