...
function l_addEntry (entry) {

	if (typeof entry !== 'string') {return false;}

	for (var i=0; i < l_entries.length; i++) {
		if (l_entries[i] === entry) {break;}
	}
...
function l_remoteEvent (server, type, para, onDone) {

	var url = server + 'event/' + type;
	LOG.warn('remoteEvent url: ' + url, 'Entry');
	UTIL.HTTPpost(url, para, (err, res, resObj) => {
		if (err) {
...
SR.API.add('queryServer', {
	...
	UTIL.contactMonitor('query/' + name.replace(/-/g, '/'), {}, (err, response) => {
		if (err) {
			LOG.error(err);
			LOG.stack();
			return onDone(err);
...
function l_updateEntryList (onDone) {
	...
	SR.API.queryServer({name: servername}, (err, result) => {
		...
		if (result && result.length > 0) {
			...
			for (var i=0; i < result.length; i++) {
				...
				if (l_addEntry(ip_port) === false) {continue;}
