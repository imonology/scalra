...
function l_query (server_name, onDone) {
	...
	if (l_servers.hasOwnProperty(server_name)) {
		...
		SR.API.queryServer({name: server_name}, (err, result) => {
...
function l_forward (server_name, conn_type, path_array, res, para, req) {
		...
		*/
		...
		UTIL.HTTPpost(
			...
			(err, response, data) => {
				...
				// TODO: possible to response websocket response directly?
				// NOTE: right now we simply return another dedicated websocket proxy port to be used later to proxy websocket directly
			},
			req.headers
		);
