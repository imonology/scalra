	...
	for (let key in obj) {
		if (typeof obj[key] === 'object') {
			if (obj[key] instanceof Array) {
				...
				var arr = [];
				for (let i=0; i < oldarr.length; i++) {arr.push(oldarr[i]);}
	...
	SR.ORM.init({
		...
		SR.ORM.read({
			...
			arr.update = function (account, data, onDone) {
				const record = arr.find((r) => {return r.account === account;});
...
var l_init = exports.init = function (args, onDone) {
	...
	for (let name in models) {
		...
		var errmsg = [];
		l_load(arr, name, model, cache, (err, result) => {
			if (err) {
