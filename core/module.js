...
var l_addStep = exports.addStep = function (step, name) {
	...
	// add to start queue
	if (l_startqueue) {l_startqueue.add(step.start, true, name);}
...
var l_get = exports.get = function (name) {
	if (l_modules.hasOwnProperty(name)) {return l_modules[name];}
	return undefined;
};
...
var l_load = exports.load = function (name, config) {
	...
	if (typeof name === 'object') {return l_loadModules(name);}
	...
	if (!loaded_module || !module) {return false;}
...
exports.stop = function (onDone) {
	...
	for (var i = l_steps.length-1; i>=0; i--) {jobqueue.add(l_steps[i].stop, true, l_steps[i].name);}
...
exports.init = function (name, config, onDone) {
	...
	if (module) {UTIL.safeCall(module.start, onDone);} else {
...
exports.dispose = function (name, onDone) {
	var module = l_get(name);
	if (module) {UTIL.safeCall(module.stop, onDone);} else {
		LOG.error('module not loaded: ' + name, l_name);
		UTIL.safeCall(onDone, false);
	}
};
...
SR.Call = function (api_name) {
	try {
		...
		if (typeof func === 'string' || typeof func === 'object') {return func;}
