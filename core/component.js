...
l_components['DB'] = exports.DB = function (collection_names, shutdown_if_fail) {
	...
	var stepDB = {
		...
		start : function (onDone) {
			SR.Module.init('DB', {collections: collection_names, shutdown_if_fail: shutdown_if_fail}, (result) => {
...
l_components['REST'] = exports.REST = function (type, handle_list, port) {
	...
	var stepRESTService = {
		...
		start : function (onDone) {
			...
			if (port && typeof port === 'string') {port = parseInt(port);}
			...
			if (handle_list && handle_list.length > 0) {
				for (var i=0; i < handle_list.length; i++) {SR.REST.addHandler(handle_list[i]);}
			}
...
l_components['FB'] = exports.FB = function () {
	...
	var step = {
		...
		start : function (onDone) {
			...
			SR.SNS.FB.start(FB_port, domain, keys, () => {
				// add individual apps to FB extension
				for (var i=0; i < app_list.length; i++) {SR.SNS.FB.add(app_list[i]);}
...
l_components['SocketIO'] = exports.SocketIO = function (type) {
	...
	var step = {
		...
		start : function (onDone) {
			...
			SR.SocketIO.start(server_or_port, () => {
				UTIL.safeCall(onDone);
			}, options);
		},
...
l_components['SockJS'] = exports.SockJS = function (type) {
	...
	var step = {
		...
		stop : function (onDone) {
			...
			if (this.created === true) {SR.SockJS.stop(type);} else {LOG.warn('SockJS [' + type + '] server not created, cannot dispose', 'SR.Component');}
...
l_components['Stream'] = exports.Stream = function () {
	...
	var step = {
		...
		stop : function (onDone) {
			...
			if (stream_server) {stream_server.stop();}
			UTIL.safeCall(onDone);
		}
	};
