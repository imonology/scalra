...
l_add('quit', 'quit current server instance', (para) => {
	SR.Settings.FRONTIER.dispose();
	return true;
});

l_add('list', 'show live app servers', (para) => {
...
l_add('start', 'start app server of a type', (para) => {
	...
	var num = 1;
	if (para.length > 2 && para[1] !== '') {num = parseInt(para[1]);}
	...
	SR.Execute.start(server_info, num, (list) => {
		if (typeof list === 'string') {LOG.warn(list, 'SR.Console');} else {
			LOG.warn(list.length + ' servers started', 'SR.Console');
			for (var i=0; i < list.length; i++) {LOG.sys(list[i], 'SR.Console');}
		}
	});
	return true;
},
...
l_add('stop', 'stop an app server of a given appID', (para) => {
	var appID = para[0];
	SR.Execute.stop(appID, (result) => {
...
l_add('stopall', 'stop all live project servers', (para) => {
	SR.Execute.stopAll();
},
'');

l_add('startall', 'restart all stopped project servers', (para) => {
	SR.Execute.startAll();
},
'');

l_add('query', 'query a list of info for live servers', (para) => {
	var owner = para[0];
	var project = para[1];
	var name = para[2];

	SR.Execute.query({owner: owner, project: project, name: name}, (list) => {
		LOG.warn(list, 'SR.Console');
	});
},
'[owner] [project] [name]');

l_add('log', 'set different display levels', (para) => {
	var level = parseInt(para[0]);
	if (isNaN(level)) {
		console.log('current level: ' + LOG.getLevel() + ' (4: system, 3: debug, 2: warning, 1: error)');
	} else if (level > 4 || level <= 0) {return false;} else {
...
l_add('info', 'display current server info [channel, cpu, server, disks, settings]', (para) => {
	let info = '';
	switch (para[0]) {
	case 'channel':
		...
		for (var i=0; i < list.length; i++) {console.log(list[i] + ': ' + SR.Comm.count(list[i]));}
		break;
...
l_add('ch', 'show all currently subscribed channels', (para) => {
	var list = SR.Comm.list();
	var output = '';
	for (var i=0; i < list.length; i++) {output += list[i] + ' ';}

	if (output !== '') {console.log(output);} else {console.log('no channels subscribed');}
	return true;
});

// show current server's config
l_add('config', 'show current server\'s config', (para) => {
	switch (para[0]) {
	case 'project':
...
l_add('show', 'show a global variable\'s states', (para) => {
	var name = para[0];
	if (!name) {console.log(SR.State.list());} else {
...
l_add('eval', 'evaluate an expression at the current server', (para, raw) => {
...
var stdinHandler = function (data) {
	...
	} else {console.log('type \'help\' for a list of commands');}
};
