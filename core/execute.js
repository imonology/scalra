...
SR.API.add('_START_SERVER', {
	...
	var onStarted = function (id) {
		...
		for (var i=0; i < l_pendingStart.length; i++) {
			...
			if (l_started.hasOwnProperty(server_type) === false) {l_started[server_type] = [];}
	...
	var start_server = function () {
		...
		if (count < size) {setTimeout(start_server, 100);}
	};
	...
	var validate_path = function (exec_path, onExec) {
		...
		SR.fs.stat(file_path, (err, stats) => {
...
function l_getServerInfo(data, times) {
	...
	SR.API._QUERY_SERVERS(data, (err, result) => {
		if (err) {
...
function l_logStartedServers(serverData) {
	...
	SR.fs.writeFile(logfile, JSON.stringify(SR.startedServers), (err) => {
		if (err) {
...
SR.API.add('_STOP_SERVER', {
	...
	if (l_id2server.hasOwnProperty(id)) {id = l_id2server[id];}

	// get server info
	SR.Call('reporting.getStat', id, (list) => {
		if (list.length === 0) {
...
var l_stop = exports.stop = function (list, project, onDone) {
	...
	if (typeof list === 'string' && list !== '') {
		list = [list];
	} else if (typeof list === 'undefined' || list.length === 0) {	// check if list exist or compose a list made of all currently registered apps servers
		list = [];
		var servers = SR.AppConn.queryAppServers();
		for (let id in servers) {list.push(id);}
	}
...
SR.API.add('_QUERY_SERVERS', {
	...
	var onResolved = function () {
		...
		SR.Call('reporting.getStat', args, (list) => {
	...
	SR.fs.realpath(fullpath, (err, resolved) => {
		if (err) {
			return (err);
		}
...
var l_query = exports.query = function (server_info, onDone) {
	...
	SR.API._QUERY_SERVERS(server_info, (err, result) => {
		if (err) {
			LOG.error(err);
			onDone([]);
		} else {
	...
	SR.DB.getData(SR.Settings.DB_NAME_SYSTEM, {},
		(re) => {
			...
			for (var c in servers) {
				if (servers[c].server.type === 'entry') {continue;}
...
var l_stopAll = exports.stopAll = function () {
	...
	l_query({}, (allServers) => {
		for (let c in allServers) {
			if (allServers[c].server.type === 'entry') {
				delete allServers[c];
			}
		}

		SR.DB.setData(SR.Settings.DB_NAME_SYSTEM, {
...
SR.Callback.onAppServerStart((info) => {
...
SR.Callback.onAppServerStop((info) => {
...
var l_run = exports.run = function (id, info, onDone, onOutput) {
	...
	*/
	...
	var onLogOpened = function (err, file_exists) {
		if (err) {
			LOG.error(
				'Failed to open log file: ' + exec_name,
				l_name
			);
			LOG.error(err, l_name);
			return;
				...
				pm2.start(
					{
						...
						script: 'npm',
						args:
							'start -- --CONNECT_MONITOR_ONSTART=true',
						output: SR.path.resolve(
							log_path,
							'output.log'
						),
						error: SR.path.resolve(
							log_path,
							'output.log'
						),
						autorestart:
					...
					(err, proc) => {
						...
						try {
							...
							SR.serverPID
								...
								|| {};
							SR.serverPID[
								`${info.owner}-${info.project}-${info.name}`
							] = proc[0].pid;
						} catch (e) {
							LOG.error(e);
							onDone(e);
	...
	log_file.open('output.log', onLogOpened, false, log_path);
};
