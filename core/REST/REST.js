...
exports.addHandler = function (handler) {
	...
	if (typeof handler === 'string') {
		...
		if (SR.fs.existsSync(fullpath) === false) {
			...
			if (SR.fs.existsSync(fullpath) === false) {fullpath = undefined;}
		}
		...
		if (fullpath) {
			...
			require(fullpath);
		} else {LOG.error('REST handler cannot be found on local or system path: ' + filename, 'SR.REST');}
	} else {
...
exports.reply = function (res, res_obj, options, headers) {
	...
	if (typeof res_obj === 'string') {
		...
		if (options.origin) {
			...
			if (UTIL.isBinary(res_obj)) {
				...
				is_binary = true;
			} else {LOG.sys('replying a text string: ' + res_obj.substring(0, 200), 'SR.REST');}
	...
	if (header['content-type']) {
		if (header['content-type'].indexOf('charset') === -1 && header['content-type'].indexOf('image') === -1) {header['content-type'] += '; charset=utf-8';}
	}
...
var l_getCookies = exports.getCookies = function (cookie_str) {
	...
	if (typeof cookie_str === 'object') {
		if (cookie_str.headers && cookie_str.headers.cookie) {cookie_str = cookie_str.headers.cookie;} else {
	...
	// To Get a Cookie
	var cookies = {};
	(typeof cookie_str === 'string') && cookie_str.split(';').forEach((cookie) => {
