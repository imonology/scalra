...
var l_getAppID = (exports.getAppID = function (connID) {
	if (l_conn2app.hasOwnProperty(connID) === false) {
		LOG.error(
			'cannot find appID via connID: ' + connID,
			'SR.AppConn.getAppID'
		);
		return undefined;
	}
...
exports.onConnect = function (conn) {
	LOG.warn(
		'incoming connection to AppManager, connID: ' + conn.connID,
		'onConnect'
	);
};
...
exports.onDisconnect = function (conn) {
	...
	if (l_conn2app.hasOwnProperty(conn.connID) === false) {
		LOG.error(
			'disconnection does not represent an app server: '
				+ conn.connID
		);
		return;
	}
...
exports.createApp = function (connID, info) {
	...
	var appInfo = {
		...
		locationID: SR.Location.getLocationID(
			info.name,
			info.local_name
		),
...
var l_deleteApp = (exports.deleteApp = function (appID, onSuccess, onFail) {
	...
	for (var i = 0; i < accounts.length; ++i) {
		...
		if (
			l_transferLobbyRequests.hasOwnProperty(accounts[i])
			=== true
		) {
			requests++;
	...
	LOG.warn(
		'delete ' + requests + ' transfer to lobby requests',
		'l_deleteApp'
	);
...
exports.checkUserThreshold = function (appID, type) {
	...
	if (l_apps.hasOwnProperty(appID) === false) {
		LOG.warn(
			'app server ['
				+ appID
				+ '] does not exist, cannot check user threshold',
			'SR.AppConn.checkUserThreshold'
		);
		return false;
	}

	if (type === 'overload') {
		let threshold = UTIL.userSettings('servers', 'overload') | 0;
		if (threshold !== 0 && l_apps[appID].usercount >= threshold) {
			return true;
		}
	} else if (type === 'underload') {
		let threshold = UTIL.userSettings('servers', 'underload');
		if (
			threshold !== undefined
			&& l_apps[appID].usercount <= threshold
		) {
			return true;
		}
	}
...
var l_getAvailableApp = (exports.getAvailableApp = function (server_name) {
	...
	LOG.sys(
		'checking available app server for ['
			+ server_name
			+ '] max_per_server: '
			+ max_count,
		'getAvailableApp'
	...
	for (var appID in l_apps) {
		...
		if (
			SR.Conn.isConnected(app.connID) === false
		...
		if (
			(max_count === 0 || app.usercount < max_count)
...
exports.getAppLocationID = function (appID) {
	return l_apps.hasOwnProperty(appID)
		? l_apps[appID].locationID
		: undefined;
};
...
var l_sendApp = (exports.sendApp = function (appID, packet_type, para) {
	...
	SR.EventManager.send(packet_type, para, [
		SR.Conn.getConnObject(connID),
	]);
});
...
exports.checkCert = function (pAccount, pToken) {
	if (
		l_certPool.hasOwnProperty(pAccount)
		&& l_certPool[pAccount].tok === pToken
	) {
